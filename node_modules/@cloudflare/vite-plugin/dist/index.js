...
var MetadataRedirectEntry = z.object({
  status: z.number(),
  to: z.string()
});
var MetadataStaticRedirects = z.record(MetadataStaticRedirectEntry);
var MetadataRedirects = z.record(MetadataRedirectEntry);
var MetadataHeaderEntry = z.object({
  set: z.record(z.string()).optional(),
  unset: z.array(z.string()).optional()
});
var MetadataHeaders = z.record(MetadataHeaderEntry);
var RedirectsSchema = z.object({
  version: z.literal(1),
...
var AssetConfigSchema = z.object({
...
function getAssetsConfig(resolvedPluginConfig, entryWorkerConfig, resolvedConfig) {
  ...
  const redirects = redirectsContents && RedirectsSchema.parse(
    constructRedirects({
  ...
  const headers = headersContents && HeadersSchema.parse(
    constructHeaders({
...
function getDotDevDotVarsContent(configPath, cloudflareEnv) {
  ...
  if (targetPath) {
    const dotDevDotVarsContent = fs5.readFileSync(targetPath);
